...
Display( )
{
	...
	//draw torus
	SetMaterial(0., 1., 0., 3000);
	glMaterialfv(GL_FRONT, GL_SPECULAR, MulArray3(.0f, White));
	glMaterialfv(GL_BACK, GL_SPECULAR, MulArray3(.0f, White));
	glutSolidTorus(.5, 1., 30, 30);
	...
	//Draw sphere, not light source

	glPushMatrix();
	glShadeModel(GL_FLAT);
	SetMaterial(0, 0, 1, 30000000);
	...
	//draw cube
	glPushMatrix();
	SetMaterial(1, .6, .3, 30);
	glTranslatef(xCube, yCube, zCube + zMove);
	glutSolidCube(.4);
	glPopMatrix();
	...
	//create red pjoint light
	glPushMatrix();
	glTranslatef(xLight0, yLight0, zLight0 + zMove);
	...
	if (Light0On) {
		glEnable(GL_LIGHT0);
		SetPointLight(GL_LIGHT0, xLight0, yLight0, zLight0 + zMove, 1, 0, 0);
	}
	...
	glPushMatrix();
	glTranslatef(xLight2, yLight2, zLight2);
	glColor3f(1.0, 1.0, .6);
	glutSolidSphere(.1, 50, 50);
	glPopMatrix();
	//glEnable(GL_LIGHTING);
	if (Light2On) {
		//cout << "Yellow Spotlight should not be making light" << endl;
		glEnable(GL_LIGHT2);
		SetSpotLight(GL_LIGHT2, xLight2, yLight2, zLight2, 0, 0, 1, 1, 1, .6);
	}
